...
#' @param adflag  A positive integer. Lag order when IC=0; maximum number of lags when
#'   IC>0.
#' @param nrep A positive integer. Number of replications
#'
...
#' @examples
#' \donttest{
#' cv <- cvPSY(100,  swindow0 = 90, IC = 0, adflag = 1, nrep = 99)
#' }

cvPSY <- function(obs, swindow0, IC, adflag, nrep) {
  ...
  MPSY <- foreach(iter = 1:m, .combine = rbind, .export = c("PSY", "ADF")) %dopar% {
